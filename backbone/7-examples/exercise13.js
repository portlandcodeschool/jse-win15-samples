...
var Router = Backbone.Router.extend({
  checkerGames: [], //all known checker games
  chessGames: [],   //all known chess games
  board: null,
  routes: {
    '': 'checkers',
    //allows optional num parameter (e.g. #checkers/7):
    'checkers(/:num)': 'checkers',
    'chess(/:num)': 'chess'
  },
  switchTo: function(game) { //game is a collection
    if (this.board) this.board.changeGame(game);
    else {
        this.board = new BoardView({collection:game});
        this.board.render();
    }
  },
  checkers: function (num) {
    if (!num) num=0;
	//...
	//this.switchTo(...)
  },
...
/* EXERCISE:
keep track of an unlimited number of games at once!
Allow any route of the form '#checkers/N' or '#chess/N'
which will set the board to the stored game N of the appropriate type.
If that game doesn't yet exist, create a new one; 
if it does, return the board to that state.
*/
